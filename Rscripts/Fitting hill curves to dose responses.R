...
# load dose-response data and subtract background-fluorescence
#replicate 1
MyFlowSet1 <- read.flowSet(path="fcs_files/dose_response_data/R1", min.limit=0.01)
chnl <- c("FSC-A", "SSC-A") #are the channels to build the gate on
...
#replicate 2
MyFlowSet2 <- read.flowSet(path="fcs_files/dose_response_data/R2", min.limit=0.01)
chnl <- c("FSC-A", "SSC-A") #are the channels to build the gate on
bou_g <- openCyto:::.boundary(MyFlowSet2[[2]], channels = chnl, min = c(0.4e5, 0.20e5), max=c(2e5,1.3e5), filterId = "Boundary Gate")
p <- autoplot(MyFlowSet2[[2]], x = chnl[1], y = chnl[2], bins=100)
p +geom_gate(bou_g)
BoundaryFrame<- Subset (MyFlowSet2[[3]], bou_g) #is the subset of cells within the gate
chnl <- c("FSC-A", "FSC-H")
sing_g <- openCyto:::.singletGate(BoundaryFrame, channels = chnl)
p <- autoplot(BoundaryFrame, x = chnl[1], y = chnl[2])
p + geom_gate(sing_g)
Singlets_MyFlowSet2<-Subset(MyFlowSet2, bou_g%&% sing_g)
medianexp<- as.data.frame(fsApply(Singlets_MyFlowSet2, each_col, median))
Transf<-linearTransform(transformationId="defaultLinearTransform", a = 1, b = -mBFP_neg)
Norm_Singlets_MyFlowSet2<- transform(Singlets_MyFlowSet2, transformList('BV421-A' ,Transf))
Transf<-linearTransform(transformationId="defaultLinearTransform", a = 1, b = -mmCherry_neg)
...
#replicate 3
MyFlowSet3 <- read.flowSet(path="fcs_files/dose_response_data/R3", min.limit=0.01)
chnl <- c("FSC-A", "SSC-A") #are the channels to build the gate on
bou_g <- openCyto:::.boundary(MyFlowSet3[[2]], channels = chnl, min = c(0.4e5, 0.20e5), max=c(2e5,1.3e5), filterId = "Boundary Gate")
p <- autoplot(MyFlowSet3[[2]], x = chnl[1], y = chnl[2], bins=100)
p +geom_gate(bou_g)
BoundaryFrame<- Subset (MyFlowSet3[[3]], bou_g) #is the subset of cells within the gate
chnl <- c("FSC-A", "FSC-H")
sing_g <- openCyto:::.singletGate(BoundaryFrame, channels = chnl)
p <- autoplot(BoundaryFrame, x = chnl[1], y = chnl[2])
p + geom_gate(sing_g)
Singlets_MyFlowSet3<-Subset(MyFlowSet3, bou_g%&% sing_g)
medianexp<- as.data.frame(fsApply(Singlets_MyFlowSet3, each_col, median))
Transf<-linearTransform(transformationId="defaultLinearTransform", a = 1, b = -mBFP_neg)
Norm_Singlets_MyFlowSet3<- transform(Singlets_MyFlowSet3, transformList('BV421-A' ,Transf))
Transf<-linearTransform(transformationId="defaultLinearTransform", a = 1, b = -mmCherry_neg)
...
medianexp$dTAG[medianexp$dTAG=="12"]<-500
medianexp$dTAG<-as.numeric(medianexp$dTAG)
#bind and calculate mCherry fold-change
rep3d4<-medianexp
rep1d4 %>% add_column(rep=1, day=4)->rep1d4
rep2d4 %>% add_column(rep=2, day=4)->rep2d4
rep3d4 %>% add_column(rep=3, day=4)->rep3d4
d4<-rbind(rep1d4,rep2d4,rep3d4)
dplyr::filter->filter
...
d4 %>% group_by(plasmid, guide.x) %>% filter(dTAG==0) %>%  summarize(max.bfp=mean(`BV421-A.x`)) ->max.bfp
...
parameters =rbind(parameters,data.frame(plasmid='SP411',K= coef(fit)[1],n=coef(fit)[2]))
...
parameters =rbind(parameters,data.frame(plasmid='SP427',K= coef(fit)[1],n=coef(fit)[2]))
...
parameters =rbind(parameters,data.frame(plasmid='SP428',K= coef(fit)[1],n=coef(fit)[2]))
...
parameters =rbind(parameters,data.frame(plasmid='SP430A',K= coef(fit)[1],n=coef(fit)[2]))
